...
test("shadowing in block", async () => {
    const silkCode = `
    let (export wasm) shadow_test = (x: i32) => i32 (
        let y = 10;
        (
            let y = 20;
            x + y
...
test("multiple exports", async () => {
    const silkCode = `
    let (export wasm) add = (x: i32) => i32 ( x + 1 );
    let (export wasm) sub = (x: i32) => i32 ( x - 1 );
    {}
    `;
    const exports = await compileAndLoad(silkCode);
...
test("arithmetic edge cases", async () => {
    const silkCode = `
    let (export wasm) div_test = (x: i32) => i32 ( 100 / x );
    {}
    `;
    const exports = await compileAndLoad(silkCode);
...
test("deeply nested bindings", async () => {
    const silkCode = `
    let (export wasm) nested_test = (x: i32) => i32 (
        let a = x + 1;
        let b = a * 2;
        let c = b + a; 
        c
...
test("if expressions evaluate and type check", async () => {
    const silkCode = `
    let (export wasm) choose = (flag: bool) => i32 (
        if flag then 10 else 20
    );
    let (export wasm) ladder = (flag: bool) => i32 (
