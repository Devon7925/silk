...
test("range operator iterates start inclusive and end exclusive", async () => {
    const silkCode = `
    ...
    const { sum_range } = await compileAndLoad(silkCode, "range_operator_sum");
    expect(sum_range(0, 5)).toBe(10);
    expect(sum_range(2, 5)).toBe(9);
    expect(sum_range(5, 5)).toBe(0);
    expect(sum_range(5, 3)).toBe(0);
}, TEST_TIMEOUT_MS);

test("range operator respects addition precedence on the end value", async () => {
    const silkCode = `
    ...
    const { sum_to } = await compileAndLoad(silkCode, "range_operator_precedence");
