...
Deno.test("while loops accumulate until reaching limit", async () => {
    ...
    const { sum_until } = (await compileToInstance(silkCode, "while_accumulate")).exports as any;
...
Deno.test("while loops stop when the guard is false", async () => {
    ...
    const { decrement_to_zero } = (await compileToInstance(silkCode, "while_decrement")).exports as any;
...
Deno.test("while with let expression stops when the pattern fails", async () => {
    ...
    const { sum_until_none } = (await compileToInstance(silkCode, "while_let_binding")).exports as any;
...
Deno.test("while break without an argument exits immediately", async () => {
    ...
    const { break_without_value } = (await compileToInstance(silkCode, "while_break_without_value")).exports as any;
