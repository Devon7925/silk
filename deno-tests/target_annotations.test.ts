...
Deno.test("allows matching target bindings", async () => {
  ...
  `;
    ...
    const { add_two } = (await compileToInstance(silkCode, "target_match")).exports as any;
...
Deno.test("allows bindings with fewer target annotations to use shared bindings", async () => {
  ...
  `;
    ...
    const { double_shared } = (await compileToInstance(silkCode, "target_shared")).exports as any;
