...
Deno.test("enum construction and matching", async () => {
    ...
    const exports = (await compileToInstance(silkCode, "enum_match")).exports as any;
...
Deno.test("enum rejects value payloads", async () => {
    ...
    const { code, stderr } = await compileExpectError(silkCode, "enum_bad_payload");
...
Deno.test("enum intrinsic can be aliased", async () => {
    ...
    const exports = (await compileToInstance(silkCode, "enum_alias")).exports as any;
...
Deno.test("enum patterns respect variant enum types", async () => {
    ...
    const exports = (await compileToInstance(silkCode, "enum_variant")).exports as any;
