...
Deno.test("compiles and runs wasm with bindings", async () => {
  ...
  `;
    ...
    const { double_add } = (await compileToInstance(silkCode, "wasm_bindings_double")).exports as any;
...
Deno.test("supports mutable assignments in wasm exports", async () => {
  ...
  `;
    ...
    const { increment_twice } = (await compileToInstance(silkCode, "wasm_bindings_mut")).exports as any;
...
Deno.test("supports boxed values in wasm exports", async () => {
  ...
  `;
    ...
    const { sum_boxed } = (await compileToInstance(silkCode, "wasm_bindings_boxed")).exports as any;
...
Deno.test("exports boxed globals as distinct memories", async () => {
  ...
  `;
    ...
    const { wasmPath, silkPath } = await compileToWasm(silkCode, "wasm_bindings_multi");
    try {
