...
Deno.test("range operator iterates start inclusive and end exclusive", async () => {
    ...
    const { sum_range } = (await compileToInstance(silkCode, "range_sum")).exports as any;
...
Deno.test("range operator respects addition precedence on the end value", async () => {
    ...
    const { sum_to } = (await compileToInstance(silkCode, "range_precedence")).exports as any;
