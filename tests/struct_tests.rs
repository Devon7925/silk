...
fn functions_accept_struct_parameters() {
    let program = "
        let rotate_pair = fn{first = first: i32, second = second: i32} -> { first = i32, second = i32 } (
            {
                first = 0 - second,
                second = first,
            }
...
fn struct_patterns_require_all_fields() {
    let program = "
        let sum_pair = fn{first = first: i32, second = second: i32} -> i32 (
            first + second
        );
...
fn functions_can_return_structs() {
    let program = "
        let make_pair = fn(x: i32) -> { first = i32, second = i32 } (
            {
                first = x,
                second = x + 1,
            }
