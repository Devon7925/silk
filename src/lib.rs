...
pub fn parse_binding(file: &str) -> Result<(Binding, &str), String> {
    ...
    let (type_hint, file) = parse_type_hint(file).map(|h| h.map(|(expr, file)| (Some(expr), file))).unwrap_or(Ok((None, file)))?;
    let file = parse_optional_whitespace(file);
    let file = parse_eq(file).ok_or("Expected = after binding pattern".to_string())?;
    let file = parse_optional_whitespace(file);
    let (expr, file) = parse_isolated_expression(file)?;
    
    Ok((Binding { pattern, type_hint, expr }, file))
}
...
let y: i32 = x
    ...
    assert_eq!(matches!(parsed[0].pattern, BindingPattern::Identifier(ref id) if id.0 == "x"), true);
    assert_eq!(matches!(parsed[0].expr, Expression::Literal(ExpressionLiteral::Number(lit)) if lit == 42), true);
    assert_eq!(matches!(parsed[1].pattern, BindingPattern::Identifier(ref id) if id.0 == "y"), true);
    assert_eq!(matches!(parsed[1].type_hint, Some(Expression::Identifier(ref hint)) if hint.0 == "i32"), true);
    assert_eq!(matches!(parsed[1].expr, Expression::Identifier(ref lit) if lit.0 == "x"), true);
}
